...
const foodName = "Apples, fuji, with skin, raw";
const keepListEntry = "Apples, fuji, with skin, raw";

console.log("Food name:         ", foodName);
console.log("Food normalized:   ", normalizeName(foodName));
console.log("");
console.log("Keep-list entry:   ", keepListEntry);
console.log("Keep-list normalized:", normalizeName(keepListEntry));
console.log("");
console.log("Match:", normalizeName(foodName) === normalizeName(keepListEntry));

// Also check the keep-list file
const keepListContent = fs.readFileSync('keep-list.txt', 'utf-8');
const keepListLines = keepListContent.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
const normalizedKeepList = keepListLines.map(normalizeName);

console.log("\nSearching keep-list for normalized food name...");
const searchFor = normalizeName(foodName);
const found = normalizedKeepList.includes(searchFor);
console.log(`Looking for: "${searchFor}"`);
console.log(`Found in keep-list: ${found}`);

if (!found) {
  console.log("\nClosest matches in keep-list:");
  normalizedKeepList.filter(k => k.includes("apple")).forEach((k, i) => {
    console.log(`  ${i + 1}. "${k}"`);
  });
}
