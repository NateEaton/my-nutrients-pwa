...
function generateModule(foods, metadata, options) {
  ...
  if (options.module) {
    ...
    output += ` * ${metadata?.description || "Multi-nutrient food database"}\n`;
    ...
    // Export the data
    output += `export const foodData = ${JSON.stringify(processedFoods, null, options.minify ? 0 : 2)};\n\n`;

    // Export metadata if not minimal
    if (!options.minimal && metadata) {
      output += `export const metadata = ${JSON.stringify(metadata, null, 2)};\n\n`;
    }

    // Export helper to get key mappings if minified
    if (options.minify) {
      output += "export const keyMappings = " + JSON.stringify(KEY_MAPPINGS, null, 2) + ";\n";
      output += "export const measureMappings = " + JSON.stringify(MEASURE_MAPPINGS, null, 2) + ";\n";
    }
  } else {
    // Generate plain JSON
